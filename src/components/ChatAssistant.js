...
import mqtt from 'mqtt'; // MQTT.js hỗ trợ cả Node.js & browser :contentReference[oaicite:1]{index=1}
import { FaRobot, FaTimes, FaPaperPlane, FaTrash } from 'react-icons/fa';

const MQTT_URI = process.env.NEXT_PUBLIC_MQTT_URI; // ví dụ: "wss://broker.emqx.io:8084/mqtt"
const TOPIC_F2 = 'home/floor2/light/set';

export default function ChatAssistant() {
  const router = useRouter();
  const clientRef = useRef(null); // dùng useRef để giữ client giữa các render
  ...
  // Hàm bật đèn tầng 2
  const turnFloor2LightOn = () => new Promise((resolve, reject) => {
  ...
  const sendMessage = async () => {
    ...
    // 🔌 Xử lý lệnh bật đèn tầng 2
    if (userInput.includes('mở đèn tầng 2')) {
  ...
  const clearMessages = () => { setMessages([]); setInput(''); };
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      {!isOpen ? (
        ...
        <div className="w-80 bg-white dark:bg-gray-900 rounded-xl shadow-2xl flex flex-col resize-y overflow-hidden" style={{ height }}>
          <div className="flex items-center justify-between p-3 bg-blue-600 text-white">
            <span className="font-semibold">Trợ lý ảo</span>
            <div className="flex gap-2">
              <button onClick={clearMessages} title="Xóa lịch sử"><FaTrash /></button>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 text-sm" style={{ maxHeight: height - 110 }}>
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex items-start gap-2 ${msg.role === 'assistant' ? 'text-left' : 'justify-end'}`}>
                {msg.role === 'assistant' && <div className="w-7 h-7 rounded-full bg-blue-500 text-white flex items-center justify-center"><FaRobot size={12} /></div>}
                <div className={`p-2 rounded-lg max-w-[80%] ${msg.role === 'assistant' ? 'bg-gray-100 text-gray-800' : 'bg-blue-600 text-white ml-auto'}`}>
                  {msg.content}
                </div>
              </div>
            ))}
          ...
          <div className="p-2 flex gap-2 border-t border-gray-300">
            ...
            />
            <button onClick={sendMessage} className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-md">
              <FaPaperPlane />
            </button>
          </div>
        </div>

...
export default function QuantityTable({ data = [] }) {
  ...
  // Calculate quantity data from devices
  const calculateQuantityData = (devices) => {
    ...
    if (devices && Array.isArray(devices)) {
      devices.forEach((deviceGroup) => {
        // Handle both direct device objects and nested ones
    ...
    //   hydro: quantityData.hydro,
    //   wind: quantityData.wind,
    //   solar: quantityData.solar,
    //   total: quantityData.total,
    //   hydroModels: quantityData.hydroModels,
    //   windModels: quantityData.windModels,
    //   solarModels: quantityData.solarModels
    // });
        ...
        <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
          {Object.entries(energyTypes).map(([key, energy]) => {
            ...
            const usagePercentage =
              totalSlots > 0 ? Math.round((energy.used / totalSlots) * 100) : 0;
              ...
              <tr key={key} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    <div
                      className={`flex-shrink-0 h-8 w-8 rounded-full ${currentColor.bgLight} ${currentColor.bgDark} flex items-center justify-center`}
                    >
    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-2xl p-6 bg-white dark:bg-gray-900/90 shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        <SummaryCard
          title="Tổng vị trí"
          value={
            energyTypes.hydro.maxSlots+
energyTypes.wind.maxSlots +
energyTypes.solar.maxSlots
          }
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <SummaryCard
          ...
          value={
            ...
            (quantityData.hydro + quantityData.wind + quantityData.solar)
            // quantityData.hydroCount - quantityData.hydro +
            // (quantityData.windCount - quantityData.wind) +
            // (quantityData.solarCount - quantityData.solar)
          }
          icon={
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="space-y-4">
          {energyTypes[selectedEnergyType].models.length > 0 ? (
            <ul className="divide-y divide-gray-200 dark:divide-gray-700">
              {Object.entries(
                countModels(energyTypes[selectedEnergyType].models)
...
function SummaryCard({ title, value, icon, color }) {
  ...
  return (
    <div
      className={`${colorClasses[color].bg} p-4 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm`}
    >
      <div className="flex items-center justify-between">
        <div>

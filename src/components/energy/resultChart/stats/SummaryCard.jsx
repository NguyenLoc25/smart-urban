...
const SummaryCard = ({ dailyData, monthlySum, mobile = false }) => {
  ...
  return (
    <div className={`
      ...
      rounded-xl shadow-md p-4 md:p-5 text-white 
      ${mobile ? '' : 'dark:from-red-600 dark:to-orange-600'}
    `}>
      <h3 className={`
        font-medium 
        ${mobile ? 'text-sm' : 'text-base md:text-lg'} 
        mb-2 md:mb-3 flex items-center
      `}>
        <svg 
          className={`
            ${mobile ? 'w-4 h-4 mr-1.5' : 'w-5 h-5 md:w-6 md:h-6 mr-2 md:mr-3'}
          `} 
          fill="none" 
          stroke="currentColor" 
          viewBox="0 0 24 24"
      ...
      <div className={`
        ${mobile ? 'text-xl' : 'text-2xl md:text-3xl'} 
        font-bold mb-1
      `}>
        {(totalProduction / 1000000).toFixed(2)} GWh
      </div>
      <div className={`
        ${mobile ? 'text-xs' : 'text-xs md:text-sm'} 

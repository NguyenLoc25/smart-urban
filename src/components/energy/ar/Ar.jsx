  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        // Scale and position Mars on the left side
        mars.scale.set(0.5, 0.5, 0.5);
        mars.position.set(-50, -10, -300); // Left side position (x negative)
    ...
    const animate = (now = performance.now()) => {
      ...
      if (sunRef.current) {
        ...
        const moveSpeed = 150 * velocity;
        const nearBoundary = 10;
        const farBoundary = -400;
        
        sunRef.current.position.z += moveSpeed * delta;
        
        if (sunRef.current.position.z > nearBoundary) {
          sunRef.current.position.z = farBoundary;
          sunRef.current.position.x = 30;
          sunRef.current.position.y = 0;
        }
      }

      // Update Mars animation
      if (marsRef.current) {
        marsRef.current.rotation.y += 0.003 * 60 * delta * velocity;
        
        // Move Mars at a slightly different speed than the sun
        const marsMoveSpeed = 120 * velocity;
        const marsNearBoundary = 10;
        const marsFarBoundary = -300;
        
        marsRef.current.position.z += marsMoveSpeed * delta;
        
        if (marsRef.current.position.z > marsNearBoundary) {
          marsRef.current.position.z = marsFarBoundary;
          // Keep Mars on the left side when resetting
          marsRef.current.position.x = -50;
          marsRef.current.position.y = -10;
        }
      }

  ...
  const ARVideo = () => {
    ...
    useEffect(() => {
      ...
      // Load earth model (placed on the left side)
      loader.load(
        '/energy/earth.glb',
        (gltf) => {
          ...
          earth.position.set(0, 0, -50); // Positioned on the left
          earth.rotation.y = Math.PI; // Rotate to face inward
          scene.add(earth);
          
          const earthLight = new THREE.PointLight(0x3399ff, 1.5, 100);
          earth.add(earthLight);
        },
        undefined,
        (error) => {
      ...
      const animate = (now = performance.now()) => {
        ...
        // earth animation (different movement pattern)
        if (earthRef.current) {
          earthRef.current.rotation.y += 0.002 * 60 * delta * velocity;
          
          const earthMoveSpeed = 80 * velocity;
          const earthNearBoundary = 20;
          const earthFarBoundary = -300;
          
          earthRef.current.position.z += earthMoveSpeed * delta;
          
          if (earthRef.current.position.z > earthNearBoundary) {
            earthRef.current.position.z = earthFarBoundary;
            earthRef.current.position.x = -40; // Maintain left position
